...
local dateparse = require("appcore/dateparse")
local geo = require("appcore/geocalcs")
...
function M.load(filename)
    ...
    if gpxdata ~= nil then
        ...
        for _, aWpt in ipairs(gpxdata:get_elements_with_name("wpt")) do
            print("Got wpt:", aWpt)
            wpt = {}
            wpt.lat = aWpt.attr.lat
            wpt.lon = aWpt.attr.lon

            waypoints[#waypoints+1] = wpt
        end
...
function M.default_format_name(tp)
...
function M.default_format_desc(tp)
...
function M:create_wpts(parent, pts, format_name, format_desc)
    local format_name = format_name or self.default_format_name
    local format_desc = format_desc or self.default_format_desc
